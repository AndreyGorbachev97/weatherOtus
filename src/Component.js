...
export class Component {
  ...
  constructor(el, state) {
    ...
    Promise.resolve().then(() => {
      this.setState(state);
      this.el.innerHTML = templateEngine(this.render(), this.state);
      this.onMount(el);
      this.setHandler();
    });
  }
  ...
  setHandler() {
    ...
    for (let i = 0; i < events.length; i += 1) {
      const foundElements = this.el.querySelectorAll(events[i].element);
      [...foundElements].forEach((foundElement) => {
        foundElement?.addEventListener(events[i].event, events[i].action);
      });
    }
  }
